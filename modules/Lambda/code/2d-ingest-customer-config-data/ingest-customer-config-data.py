...
def lambda_handler(event, context):
    ...
    try:
        ...
        ### Fetch only those customer's data, whose config bucket information is not NULL

        result = onBoardingTable.scan(
            FilterExpression=Attr("configBucketName").exists() & 
            Attr("configBucketPath").exists()
        )
        ...
        while isDbDataPending:
            ...
            if isDbDataPending:
                result = onBoardingTable.scan(FilterExpression=Attr("configBucketName").exists() & 
                        Attr("configBucketPath").exists(),
...
def sync_buckets(item):
    ...
    destinationBucketPath = item['cuid'] + ".cc-aws-data/" + item['cuid'] + ".cc-aws-data-config/"
    ...
    sourceBucketName = item['configBucketName']
    sourceBucketPath = "" if item['configBucketPath'] == '/' else item['configBucketPath']
...
def generate_date_ranges(prevDays):
    ...
    ### Generate the previous days ranges in format of config data.
...
def get_account_id(item):
    accountId = ""
    if 'configBucketAccountId' in item and item['configBucketAccountId'] != "" and item['configBucketAccountId'] != None:
        accountId = item['configBucketAccountId']
    else:
