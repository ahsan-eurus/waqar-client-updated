...
def lambda_handler(event, context):
    ...
    try:
        ...
        ### Fetch only those customer's data, whose guardDuty bucket information is not NULL

        result = onBoardingTable.scan(
            FilterExpression=Attr("guardDutyBucketName").exists() &
            Attr("guardDutyBucketPath").exists()
        )
        ...
        while isDbDataPending:
            ...
            if isDbDataPending:
                result = onBoardingTable.scan(FilterExpression=Attr("guardDutyBucketName").exists() &
                        Attr("guardDutyBucketPath").exists(),
...
def sync_buckets(item):
    ...
    destinationBucketPath = item['cuid'] + ".cc-aws-data/" + item['cuid'] + ".cc-aws-data-guardduty/"
    ...
    sourceBucketName = item['guardDutyBucketName']
    sourceBucketPath = "" if item['guardDutyBucketPath'] == '/' else item['guardDutyBucketPath']
...
def generate_date_ranges(prevDays):
    ...
    ### Generate the previous days ranges in format of guardDuty data.
...
def get_account_id(item):
    accountId = ""
    if 'guardDutyBucketAccountId' in item and item['guardDutyBucketAccountId'] != "" and item['guardDutyBucketAccountId'] != None:
        accountId = item['guardDutyBucketAccountId']
    else:
